 ...
 *   codegraph search <search>    Search for symbols (alias for query)
 ...
 *   codegraph explore <query>    Explore relevant symbols' source + call paths
 ...
 *
 * The CLI face of the MCP codegraph_search tool: quick symbol search by name.
 * Same handler as `codegraph query` — exists so agents that call `search`
 * (instead of `query`) find the right command.
 */
program
  .command('search <search>')
  .description('Quick symbol search by name (alias for query). Returns locations only (no code).')
  .option('-p, --path <path>', 'Project path')
  .option('-l, --limit <number>', 'Maximum results', '10')
  .option('-k, --kind <kind>', 'Filter by node kind (function, class, variable, etc.)')
  .option('-j, --json', 'Output as JSON')
  .action(async (search: string, options: { path?: string; limit?: string; kind?: string; json?: boolean }) => {
    const projectPath = resolveProjectPath(options.path);
    ...
    try {
      ...
      const result = await handler.execute('codegraph_search', args);

      if (options.json) {
        const cgResults = cg.searchNodes(search, {
          limit: parseInt(options.limit || '10', 10),
          kinds: options.kind ? [options.kind as any] : undefined,
        });
        console.log(JSON.stringify(cgResults.map((r: any) => ({
          name: r.node.name,
          kind: r.node.kind,
 ...
 * codegraph explore <query...>
 *
 * The CLI face of the MCP codegraph_explore tool: relevant symbols\' source +
 * call paths in one shot. Allows agents without MCP tools to reach the graph
 * through a plain shell command.
 */
program
  .command('explore <query...>')
  .description('Explore an area: relevant symbols\' source + call paths in one shot (same output as the codegraph_explore MCP tool)')
...
program
  ...
  .action(async (name: string, options: { path?: string; file?: string; offset?: string; limit?: string; symbolsOnly?: boolean }) => {
    ...
    try {
      ...
      // Detect file mode: explicit --file or path contains separator
      const isFileMode = !!(options.file || name.includes('/') || name.includes('\\'));
      const isSymbolWithDisambiguation = !!(options.file && name && name !== options.file);

      if (isFileMode && !isSymbolWithDisambiguation) {
        // FILE MODE: read source from disk with line numbers + blast radius
        const filePath = options.file || name.replace(/\\/g, '/');
        ...
        // Blast-radius header
        const deps = cg.getFileDependents(filePath);
        if (deps.length > 0) {
          console.log(chalk.bold(`File: ${filePath}`));
          console.log(chalk.dim(`Dependents: ${deps.length} file(s)`));
          for (const d of deps.slice(0, 15)) console.log(chalk.dim(`  - ${d}`));
          if (deps.length > 15) console.log(chalk.dim(`  ... +${deps.length - 15} more`));
          console.log();
        }

        const startLine = offset || 1;
        const endLine = limit ? startLine + limit - 1 : lines.length;
        const linePad = String(endLine).length;

        for (let i = startLine - 1; i < Math.min(endLine, lines.length); i++) {
          console.log(`${String(i + 1).padStart(linePad)}\t${lines[i]}`);
        }
      ...
      // SYMBOL MODE
      const args: Record<string, unknown> = { symbol: name, includeCode: true };
      ...
      error(`Node command failed: ${err instanceof Error ? err.message : String(err)}`);
      process.exit(1);
    }
  });
